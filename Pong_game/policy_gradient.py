...
def compute_cost(readout,action_holder,reward_holder):
	...
	Args:
		...
		reward_holder: discounted rewards
	Returns:
		loss
	"""

	zero = tf.constant(0, dtype=tf.float32)
	where = tf.not_equal(action_holder, zero)
	a_indices = tf.where(where)
	print(a_indices.get_shape())




	prob =tf.gather_nd(params=readout, indices=a_indices)
	loss = tf.reduce_mean(-(tf.log(prob) * reward_holder))
...
def get_action_index(readout_t,epsilon,t):
	...
	"""
	pos=np.random.random(size=1)
	c=np.ndarray.tolist(readout_t)




	if(pos>epsilon and t>OBSERVE):
		action_index = np.random.choice(len(readout_t),1, c)[0]
	else:
         action_index=np.random.randint(0,len(readout_t),1)[0]
...
def run_selected_action(a_t,s_t,game_state):
	...
	s_t1 = np.stack((x_t, s_t[:, :, 1], s_t[:, :, 2], s_t[:, :, 3]), axis=2)
