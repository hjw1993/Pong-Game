...
def compute_cost(target_q,a_t,q_value):
	...
	#a_t an vector of actions

	zero = tf.constant(0, dtype=tf.float32)
	where = tf.not_equal(a_t, zero)
	...
	q_eval = tf.gather_nd(params=q_value, indices=a_indices)  # shape=(None, )




	# TO DO: Q-Learning Cost.
	cost=tf.reduce_mean(tf.squared_difference(target_q, q_eval))
...
def compute_target_q(next_state_batch,r_batch,readout_j1_batch,minibatch):
	...
	target_q_batch = r_batch + GAMMA * np.amax(readout_j1_batch, axis=1)


	for i in range (0,len(minibatch)):
		if (minibatch[i][4]):
			target_q_batch[i]=r_batch[i]
